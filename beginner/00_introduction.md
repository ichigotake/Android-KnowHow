...
各種用語の詳細な説明はそこら中に溢れているので、お好みの入門書や師匠から学ぶ際のヒントとして。
...
独自ルール・独自実装で頑張るよりは、一般的な Intent のルールに則ったり、通知を積極的に利用したりする **だけ** のアプリなら実装も簡単で、かつ使いやすいアプリになりやすい。

今Playストアに溢れているアプリは、暗黙的 Intent 、通知等のエコシステムを利用していないものも多くあり、始めに作るアプリとしては「いつもやってる事のタップ数や考える事を減らすアプリ」を作ると考えると、お手軽に便利アプリを作れるのでオススメ。

少しでも難しい実装があったら、「これは今ほんとうに必要なものなのか」「既存の環境に乗っかれる別のやり方があるのではないか」などを考える機会だと思うようにするとよい。
...
- プレビューを自前で実装せず、外部の WEB ブラウザに委譲する
    - 「通知上からプレビュー画面を呼び出す」という体験は自前実装も委譲も同じで、それならユーザの環境にあるものを使う方がエコ


### 辞書アプリ

この文書の一番最初に示した以下の内容を実現する例を考える
...
よく使う単語の一覧を表示するだけの画面を用意して、それを Twitter に投稿するだけのアプリを作りたい場合を例に考える。

実装するにあたって、Twitter のAPI を利用して投稿するライブラリを用意して開発者ページからアプリ登録をして…という考えになると思うが、 **これはとても大変**

**まずは** 「汎用的に使えるアプリを利用者がたまたまTwitterで使っているだけ」というような利用ケースを考えるとよい。

それを踏まえて、実装するために考える事としては以下のようなものが考えられる。

- アプリの機能としては「テキストを他のアプリに送る」のみで「外部サービスへ投稿はしない」
    - 投稿は送った先のアプリがやること
    - 一般的な Intent に則ってないアプリは Android アプリらしいとは言えないという考え方も出来るので、暗黙的 Intent で出ないアプリは切り捨てるのも一つの選択
- Activity で単語の一覧を表示する
- 選択した単語を暗黙的 Intent の「共有」で他のアプリに送る
- ほんとによく使うものであるならば、「消せない通知」を常に出しておき、いつでも簡単に Activity を起動出来るようにするのも一興

もちろん Twitter への投稿に自作アプリ上から Twitter へ投稿する機能もあってもよい。それはアプリの方向性としてどこを目指したいか次第。

 `暗黙的Intent` を利用する事がオススメなのは、最初のステップとして実装が比較的楽だというだけでなく、そうした方がユーザが Twitter への投稿以外の用途でも使いたいとなった際の潰しが効きやすい事。
 

小難しく考えずとも、AndroidSDK のエコシステムに乗っかるだけでも汎用性のあるいい感じのアプリは作れますよ、という事も忘れないでおきたい。

参考情報として、暗黙的 Intent を利用する際は、 [android-intents](https://github.com/d-tarasov/android-intents) というライブラリを使うと簡単に暗黙的 Intent を呼び出せる。ライブラリが使えなくても [中身を見ると](https://github.com/d-tarasov/android-intents/blob/master/library/src/com/dmitriy/tarasov/android/intents/IntentUtils.java) 参考になる事もあるはず。
